...
def count_chunk_elements1(fname, chunksize=1000000, max_chunk=None, do_process_chunk=True):

    symbol_roots = Counter()

    for (i,chunk) in enumerate(islice(raw_taq.TAQ2Chunks(fname, 
                                                         chunksize=chunksize, 
                                                         do_process_chunk=do_process_chunk), max_chunk)):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-chunksize', dest='chunksize', action='store', help='chunksize', default=1000000)
    ...
    print ("max_chunk:{} chunksize:{}".format(max_chunk, chunksize))
    ...
    h5_path = "/global/scratch/ryee/symbol_count/output2/{year:04d}{month:02d}{day:02d}.h5".format(year=year, 
               month=month, day=day) 
    timing_path = "/global/scratch/ryee/symbol_count/timing2/{year:04d}{month:02d}{day:02d}.txt".format(year=year, 
               month=month, day=day)     


    if not os.path.exists(h5_path):

        c = count_chunk_elements1(args.fname, chunksize=chunksize,  max_chunk=max_chunk)
