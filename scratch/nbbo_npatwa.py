...
print nbbo.sortByKey().take(100)

# find average values over a second of best bid and best ask
bbsec = bestbid.map(lambda rec: (rec[0] [0:6], rec[1]))
bbavg = bbsec.mapValues(lambda x: (x, 1)).reduceByKey(lambda x, y: (x[0] + y[0], x[1] + y[1]))
bbavg = bbavg.map(lambda rec: (rec[0], (rec[1] [0])/(rec[1] [1])))

basec = bestask.map(lambda rec: (rec[0] [0:6], rec[1]))
baavg = basec.mapValues(lambda x: (x, 1)).reduceByKey(lambda x, y: (x[0] + y[0], x[1] + y[1]))
baavg = baavg.map(lambda rec: (rec[0], (rec[1] [0])/(rec[1] [1])))
